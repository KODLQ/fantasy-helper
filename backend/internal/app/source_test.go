...
func TestSourceNormalizesSnapshot(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/bootstrap-static/":
			_, _ = w.Write([]byte(`{"events":[{"id":1,"name":"Gameweek 1","is_current":true}],"teams":[{"id":1,"name":"Example","short_name":"EXA"}],"elements":[{"id":10,"first_name":"A","second_name":"Player","web_name":"Player","element_type":3,"team":1,"now_cost":55,"total_points":42,"form":"6.2","minutes":500,"value_form":"11.1","status":"a"}]}`))
		case "/fixtures/":
	...
	defer server.Close()
	season, weeks, teams, players, fixtures, checksum, err := NewFPLSource(server.URL).Snapshot(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if season.ID != 1 || len(weeks) != 1 || len(teams) != 1 || len(players) != 1 || len(fixtures) != 1 || checksum == "" {
