...
export type AnalysisCoverage = { requested: number; selected: number; complete: number; omitted: number; missingEntryIds: number[] };
export type LeagueAnalysisSummary = { leagueId: number; name: string; seasonId: number; gameweek: number; members: LeagueStandings['members']; selectedEntryIds: number[]; omittedEntryIds: number[]; snapshotIds: Record<number, string>; coverage: AnalysisCoverage; outcomeState: string; formulaVersions: string[]; missingInputs: string[]; warnings: string[] };
export type LeagueAnalysisComparison = Omit<LeagueComparison, 'comparisons'> & { snapshotIds: Record<number, string>; coverage: AnalysisCoverage; formulaVersions: string[]; warnings: string[]; comparisons: (LeagueComparison['comparisons'][number] & { startingXIOverlap: number; differentialContribution: number; captainDelta: number })[] };
export type GameweekAutopsy = { entryId: number; rivalEntryId?: number; seasonId: number; gameweek: number; grossPoints: number; transferCost: number; netPoints: number; captainId?: number; captainDelta: number; benchPoints: number; activeChip?: string; transfers: { gameweek: number; playerIn: number; playerOut: number; playerInCost: number; playerOutCost: number; madeAt: string }[]; automaticSubstitutions: { playerIn: number; playerOut: number; impact: number }[]; contributions: { playerId: number; basePoints: number; multiplier: number; effectivePoints: number }[]; rivalComparison?: LeagueAnalysisComparison['comparisons'][number]; outcomeState: string; snapshotIds: Record<number, string>; formulaVersions: string[]; missingInputs: string[]; warnings: string[]; metricsAvailable: boolean; unfinishedFixtureIds: number[] };
...
export const api = {
  ...
  leagueAnalysisSummary: (seasonId: number, gameweek: number, leagueId: number, memberLimit: number, signal?: AbortSignal) => request<LeagueAnalysisSummary>(`/api/v1/analysis/leagues/${leagueId}/summary?seasonId=${seasonId}&gameweek=${gameweek}&memberLimit=${memberLimit}`, { signal, operation: 'league-analysis-summary', staleKey: `league-analysis-summary:${seasonId}:${gameweek}`, expectedSeasonId: seasonId }),
  leagueAnalysisComparison: (seasonId: number, gameweek: number, leagueId: number, entryIds: number[], signal?: AbortSignal) => request<LeagueAnalysisComparison>(`/api/v1/analysis/leagues/${leagueId}/comparison?seasonId=${seasonId}&gameweek=${gameweek}&entryIds=${entryIds.join(',')}&memberLimit=8`, { signal, operation: 'league-analysis-comparison', staleKey: `league-analysis-comparison:${seasonId}:${gameweek}`, expectedSeasonId: seasonId }),
  gameweekAutopsy: (seasonId: number, gameweek: number, entryId: number, rivalEntryId?: number, signal?: AbortSignal) => request<GameweekAutopsy>(`/api/v1/analysis/gameweeks/${gameweek}/autopsy?seasonId=${seasonId}&entryId=${entryId}${rivalEntryId ? `&rivalEntryId=${rivalEntryId}` : ''}`, { signal, operation: 'gameweek-autopsy', staleKey: `gameweek-autopsy:${seasonId}:${gameweek}:${entryId}`, expectedSeasonId: seasonId }),
};
