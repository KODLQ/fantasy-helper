...
type View = 'research' | 'compare' | 'squad' | 'recommendations' | 'manager';
...
function App() {
  ...
  return <div className="app-shell" data-testid="app-shell">
    <aside className="sidebar">
      ...
      <nav>{([['research', 'Research', '⌕'], ['compare', `Compare${compareIDs.length ? ` · ${compareIDs.length}` : ''}`, '◫'], ['squad', 'Squad planner', '♙'], ['recommendations', 'Recommendations', '✦'], ['manager', 'Manager & leagues', '◎']] as [View, string, string][]).map(([key, label, icon]) => <button key={key} className={view === key ? 'nav-item active' : 'nav-item'} onClick={() => setView(key)}><span>{icon}</span>{label}</button>)}</nav>
      <div className="sidebar-footer"><SyncControl onNotice={setNotice} onStatus={setFreshness} /></div>
    </aside>
    <main className="main-content">
      <header className="topbar"><div><span className="eyebrow">FPL / {season?.name ?? 'select season'}</span><h1>{view === 'research' ? 'Player research' : view === 'compare' ? 'Compare players' : view === 'squad' ? 'Squad planner' : view === 'manager' ? 'Manager & leagues' : 'Recommendations'}</h1></div><div className="topbar-actions"><SeasonSelector /><div className="week-chip">{gameweek ? `GW ${gameweek}` : 'No gameweek'} <span>·</span> {season?.state === 'historical' ? 'historical' : 'decision window'}</div><ProfileMenu open={profileOpen} onOpenChange={setProfileOpen} /></div></header>
