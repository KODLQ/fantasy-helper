...
import { api, Player, positionName } from '../api';
...
export function Research({ seasonId, onSelect, onCompare, onSquad }: { seasonId: number; onSelect: (id: number) => void; onCompare: (id: number) => void; onSquad: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const cacheKey = `fantasy-helper:players:${seasonId}:${params.toString()}`;
      const cached = window.localStorage.getItem(cacheKey);
      if (cached) {
        const saved = JSON.parse(cached) as { items: Player[]; total: number };
        setPlayers(saved.items); setTotal(saved.total); setLoading(false);
      }
      setLoading(true);
      api.players(params, controller.signal).then((result) => {
        setPlayers(result.items); setTotal(result.total); setError('');
        window.localStorage.setItem(cacheKey, JSON.stringify({ items: result.items, total: result.total }));
  ...
  const columns = useMemo<DataTableColumn<Player>[]>(() => [
    ...
    { id: 'club', header: 'Club', filter: { type: 'select', id: 'teamId', label: 'Filter by club', options: clubOptions }, cell: (player) => clubName(player.teamId) },
    ...
    { id: 'actions', header: <span className="visually-hidden">Actions</span>, cell: playerActions },
  ], [onCompare, onSelect]);
