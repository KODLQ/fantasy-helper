...
- [x] 2.5 Add sanitized fixtures and tests for valid, missing, unauthorized, expired-session, and partial responses.
- [x] 2.6 Add provenance/checksum/normalization-version fields and source-conflict representation.
- [x] 2.7 Add secret-provider tests proving no cookie/password/token enters database rows, payloads, logs, traces, browser storage, or error responses.

## 3. Manager synchronization

- [x] 3.1 Add independent manager sync runs, stages, work items, retry policy, and freshness status.
...
- [x] 3.8 Add synchronized member-pick work items linked to reusable entry/gameweek snapshots.
...
- [x] 4.4 Add sanitized multi-page fixtures and tests for page errors, closed leagues, and permission failures.
- [x] 4.5 Add league member identity/link tables and deterministic standings-to-entry reconciliation.
...
- [x] 5.2 Add decision-analysis joins for points, captain multiplier, bench points, transfer cost, and chips.
...
- [x] 5.5 Add explicit import-as-draft and replace-planning-squad actions with atomic validation and source provenance.
- [x] 5.6 Add frontend controls for connect, sync, preview, import, and replace confirmation states.
- [x] 5.7 Add league-member pick synchronization with configurable member limits, selected-gameweek scope, and bounded concurrency.
...
- [x] 5.9 Add actual/provisional/estimated point-difference calculations joined to public player-gameweek facts.
...
- [x] 5.11 Add integration tests for multiple entries, multiple leagues, pagination resume, member-pick failures, public-data joins, import validation, no-overwrite behavior, and point-difference labeling.
- [x] 5.12 Document configuration, privacy/retention, session setup, comparison limits, estimate limitations, and the non-mutating scope of the feature.
- [x] 5.13 Add contract tests for missing scope, source conflicts, deterministic member selection, and provenance.
- [x] 5.14 Add Playwright coverage for connect/login, manager sync progress/errors, active-team preview/import, league selection, member limits, comparison metrics, omitted members, and no-overwrite confirmation states.
- [x] 5.15 Add privacy integration tests for per-user isolation, disconnect/revocation, private export/deletion, retention cleanup, and public-fact preservation.
